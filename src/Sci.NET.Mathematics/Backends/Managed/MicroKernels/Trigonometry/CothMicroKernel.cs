...
namespace Sci.NET.Mathematics.Backends.Managed.MicroKernels.Trigonometry;

[SuppressMessage("Roslynator", "RCS1158:Static member in generic type should use a type parameter", Justification = "By design")]
internal class CothMicroKernel<TNumber> : IUnaryOperation<TNumber>, IUnaryOperationAvx2
    where TNumber : unmanaged, INumber<TNumber>, IHyperbolicFunctions<TNumber>
{
    ...
    {
        return TNumber.Cosh(input) / TNumber.Sinh(input);
    }

    [MethodImpl(ImplementationOptions.HotPath)]
    public static float ApplyScalarFp32(float input)
    {
        return MathF.Cosh(input) / MathF.Sinh(input);
    }
