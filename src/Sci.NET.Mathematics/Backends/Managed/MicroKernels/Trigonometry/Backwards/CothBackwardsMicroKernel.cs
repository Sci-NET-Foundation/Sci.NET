...
internal class CothBackwardsMicroKernel<TNumber> : IBinaryOperation<TNumber>, IBinaryOperationAvx2
    where TNumber : unmanaged, INumber<TNumber>, IHyperbolicFunctions<TNumber>
{
    ...
    {
        var csch = TNumber.One / TNumber.Sinh(left);

        return right * -(csch * csch);
    }

    [MethodImpl(ImplementationOptions.HotPath)]
    public static float ApplyScalarFp32(float left, float right)
    {
        var csch = 1.0f / MathF.Sinh(left);
