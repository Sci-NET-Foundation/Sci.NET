...
internal class Cot2BackwardsMicroKernel<TNumber> : IBinaryOperation<TNumber>, IBinaryOperationAvx2
    where TNumber : unmanaged, INumber<TNumber>, ITrigonometricFunctions<TNumber>
{
    ...
    public static float ApplyScalarFp32(float left, float right)
    {
        ...
        var cot = cos / sin;
        var csc2 = 1.0f / (sin * sin);

        return right * -2.0f * csc2 * cot;
    }
    ...
    public static double ApplyScalarFp64(double left, double right)
    {
        ...
        var cot = cos / sin;
        var csc2 = 1.0d / (sin * sin);

        return right * -2.0d * csc2 * cot;
    }
