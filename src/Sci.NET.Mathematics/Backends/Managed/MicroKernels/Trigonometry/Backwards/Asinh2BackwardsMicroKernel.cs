...
internal class Asinh2BackwardsMicroKernel<TNumber> : IBinaryOperation<TNumber>, IBinaryOperationAvx2
    where TNumber : unmanaged, INumber<TNumber>, IRootFunctions<TNumber>, IHyperbolicFunctions<TNumber>
{
    ...
    public static TNumber ApplyScalar(TNumber left, TNumber right)
    {
        ...
        var asinh = TNumber.Asinh(left);
        var sqrtOnePlusX2 = TNumber.Sqrt(TNumber.One + (left * left));

        return right * (two * asinh / sqrtOnePlusX2);
    }

    [MethodImpl(ImplementationOptions.HotPath)]
    public static float ApplyScalarFp32(float left, float right)
    {
        var asinh = MathF.Asinh(left);
        var sqrt = MathF.Sqrt(1.0f + (left * left));
        return right * (2.0f * asinh / sqrt);
    }

    [MethodImpl(ImplementationOptions.HotPath)]
    public static double ApplyScalarFp64(double left, double right)
    {
        var asinh = Math.Asinh(left);
        var sqrt = Math.Sqrt(1.0d + (left * left));
        return right * (2.0d * asinh / sqrt);
    }
