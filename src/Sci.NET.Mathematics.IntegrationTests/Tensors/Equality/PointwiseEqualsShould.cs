...
public class PointwiseEqualsShould : IntegrationTestBase
{
    ...
    {
        PerformScalarTest<BFloat16>(device, true);
        PerformScalarTest<BFloat16>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenFp16Scalar(IDevice device)
    {
        PerformScalarTest<Half>(device, true);
        PerformScalarTest<Half>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenFp32Scalar(IDevice device)
    {
        PerformScalarTest<float>(device, true);
        PerformScalarTest<float>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenFp64Scalar(IDevice device)
    {
        PerformScalarTest<double>(device, true);
        PerformScalarTest<double>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenI8Scalar(IDevice device)
    {
        PerformScalarTest<sbyte>(device, true);
        PerformScalarTest<sbyte>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenU8Scalar(IDevice device)
    {
        PerformScalarTest<byte>(device, true);
        PerformScalarTest<byte>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenI16Scalar(IDevice device)
    {
        PerformScalarTest<short>(device, true);
        PerformScalarTest<short>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenU16Scalar(IDevice device)
    {
        PerformScalarTest<ushort>(device, true);
        PerformScalarTest<ushort>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenI32Scalar(IDevice device)
    {
        PerformScalarTest<int>(device, true);
        PerformScalarTest<int>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenU32Scalar(IDevice device)
    {
        PerformScalarTest<uint>(device, true);
        PerformScalarTest<uint>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenI64Scalar(IDevice device)
    {
        PerformScalarTest<long>(device, true);
        PerformScalarTest<long>(device, false);
    }
    [Theory]
    [MemberData(nameof(ComputeDevices))]
    public void ReturnCorrectResult_GivenU64Scalar(IDevice device)
    {
        PerformScalarTest<ulong>(device, true);
        PerformScalarTest<ulong>(device, false);
    }
    ...
    private static void PerformScalarTest<TNumber>(IDevice device, bool isDifferent)
        where TNumber : unmanaged, INumber<TNumber>
    {
        // Arrange
        using var left = new Scalar<TNumber>(TNumber.CreateChecked(5), device.GetTensorBackend(), true);
        using var right = new Scalar<TNumber>(isDifferent ? TNumber.CreateChecked(10) : TNumber.CreateChecked(5), device.GetTensorBackend(), true);
        using var expectedResult = new Scalar<TNumber>(isDifferent ? TNumber.Zero : TNumber.One, device.GetTensorBackend());
