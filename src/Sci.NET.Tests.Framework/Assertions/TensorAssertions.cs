    ...
    where TNumber : unmanaged, INumber<TNumber>
{
    ...
    /// <param name="tolerance">The tolerance for the comparison.</param>
    /// <returns>A <see cref="AndConstraint{TAssertions}" /> object.</returns>
    public AndConstraint<TensorAssertions<TNumber>> NotHaveApproximatelyEquivalentElements(Array values, TNumber tolerance)
    {
        if (Subject.IsScalar())
        {
            _ = Execute
    ...
    /// <summary>
    /// Asserts that all elements of the tensor are within the specified range.
    /// </summary>
    /// <param name="min">The minimum value.</param>
    /// <param name="max">The maximum value.</param>
    /// <returns>A <see cref="AndConstraint{TAssertions}" /> object.</returns>
    public AndConstraint<TensorAssertions<TNumber>> BeInRange(TNumber min, TNumber max)
    {
        _ = Execute
            .Assertion
            .BecauseOf(string.Empty)
            .Given(() => Subject.Memory.ToArray())
            .ForCondition(tensorElements => tensorElements.All(x => x >= min && x <= max))
            .FailWith("Expected all tensor elements to be in range {0}-{1}{reason}, but found elements out of range.", min, max);
    ...
    {
        if (tensor1.GetValue(indices) is T leftItem && tensor2.GetValue(indices) is T rightItem)
        {
            if (T.IsPositiveInfinity(leftItem) && T.IsPositiveInfinity(rightItem))
            {
                return true;
            }

            if (T.IsNegativeInfinity(leftItem) && T.IsNegativeInfinity(rightItem))
            {
                return true;
            }

            if (T.IsNaN(leftItem) && T.IsNaN(rightItem))
            {
                return true;
            }
        }
